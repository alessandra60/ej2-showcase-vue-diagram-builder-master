<template>
  <div>
    <ejs-dropdownbutton
      ...
      v-on:select="drawConnectorChange($event)"
    ></ejs-dropdownbutton>
  </div>
</template>
<script lang="ts">
...
import { Diagram, DiagramPlugin } from "@syncfusion/ej2-vue-diagrams";
import {
...
export default class User extends Vue {
  ...
  public drawConnectorChange(args: MenuEventArgs): void {
    ...
    (document.getElementById("btnDrawConnector") as any).classList.add(
      "tb-item-selected"
    );
  }
  public removeSelectedToolbarItem(): void {
    ...
    (document.getElementById("btnDrawShape") as any).classList.remove(
      "tb-item-selected"
    );
    (document.getElementById("btnDrawConnector") as any).classList.remove(
      "tb-item-selected"
    );
  }
}
