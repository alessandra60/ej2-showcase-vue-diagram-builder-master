...
export class MindMap {
    ...
    let commandManager: CommandManagerModel = {
      commands: [
        ...
        },
        {
          ...
          canExecute: this.canExecute,
          execute: this.addSibilingChildTop.bind(this),
          name: "sibilingChildTop",
        },
        {
          gesture: { key: Keys.Enter, keyModifiers: KeyModifiers.Shift },
          canExecute: this.canExecute,
          execute: this.addSibilingChildBottom.bind(this),
          name: "sibilingChildBottom",
        },
        {
        ...
        },
        {
          ...
          canExecute: this.canExecute,
          execute: this.expandCollapse.bind(this),
          name: "expandCollapse",
        },
        {
          gesture: { key: Keys.F2 },
          canExecute: this.canExecute,
          execute: this.editNode.bind(this),
          name: "editing",
        },
        {
          gesture: { key: Keys.F1 },
          canExecute: this.canExecute,
          execute: MindMapUtilityMethods.onHideNodeClick.bind(
        ...
        },
        {
          gesture: { key: Keys.F8 },
          canExecute: this.canExecute,
          execute: this.onFit.bind(this),
          name: "FitToPage",
        },
        {
          gesture: { key: Keys.E, keyModifiers: KeyModifiers.Control },
          canExecute: this.canExecute,
          execute: this.expandCollapseParent.bind(this),
  ...
  private removeChild(args: { [key: string]: Object }): void {
    ...
    if (
      (diagram.selectedItems as any).nodes.length > 0 &&
      (diagram.selectedItems as any).nodes[0].id !== "rootNode"
    ) {
  ...
  public underlineLabel() {
    ...
    if (diagram.selectedItems.nodes && diagram.selectedItems.nodes.length > 0) {
      ...
      if (
        ...
        node.annotations[0].style.textDecoration =
  ...
  private navigateChild(direction: string): void {
    ...
    if (direction === "top" || direction === "bottom") {
      ...
      let index: number = sameLevelNodes.indexOf(
        (diagram.selectedItems as any).nodes[0] as Node
      );
      node =
        direction === "top"
  ...
  private getSameLevelNodes(): Node[] {
    ...
    if ((diagram.selectedItems.nodes as any).length > 0) {
      ...
      let orientation: string = (
        node.addInfo as { [key: string]: Object }
      ).orientation.toString();
      let connector: Connector = MindMapUtilityMethods.getConnector(
        diagram.connectors,
        node.inEdges[0]
      );
      let parentNode: Node = MindMapUtilityMethods.getNode(
        diagram.nodes,
        connector.sourceID
      );
      for (let i: number = 0; i < parentNode.outEdges.length; i++) {
        connector = MindMapUtilityMethods.getConnector(
          diagram.connectors,
        ...
        if (childNode) {
          let childOrientation: string = (
            childNode.addInfo as { [key: string]: Object }
          ).orientation.toString();
  ...
  private getMinDistanceNode(diagram: Diagram, direction: string): Node {
    ...
    let leftOrientationFirstChild: any = null,
      rightOrientationFirstChild: any = null;
    if (node.id === "rootNode") {
      ...
      for (let i: number = 0; i < edges.length; i++) {
        ...
        childNode = MindMapUtilityMethods.getNode(
          diagram.nodes,
          connector.targetID
        );
        let addInfo: { [key: string]: Object } = childNode.addInfo as {
          [key: string]: Object;
        };
        if (addInfo.orientation.toString().toLowerCase() === direction) {
          ...
          if (
            ...
            (childBounds.top >= oldChildBoundsTop || oldChildBoundsTop === 0)
          ) {
      ...
      if (!lastChildNode) {
        lastChildNode =
          direction === "left"
            ? leftOrientationFirstChild
            : rightOrientationFirstChild;
      }
    } else {
      let edges: string[] = [];
      ...
      for (let i: number = 0; i < edges.length; i++) {
        ...
        childNode = MindMapUtilityMethods.getNode(
          diagram.nodes,
          selecttype === "target" ? connector.targetID : connector.sourceID
        );
  ...
  public createMindMap(isNew: boolean): void {
    ...
    if (isNew) {
      ...
      diagram.selectedItems = {
        userHandles: MindMapUtilityMethods.handle,
      ...
      diagram.snapSettings.constraints =
        (diagram.snapSettings as any).constraints & ~SnapConstraints.ShowLines;
      diagram.constraints = diagram.constraints | DiagramConstraints.UndoRedo;
      diagram.tool = DiagramTools.SingleSelect | DiagramTools.ZoomPan;
      diagram.dataBind();
  ...
  public getShortCutKeys(
    shortcutKeys: { [key: string]: Object }[]
  ): ShapeAnnotationModel[] {
    ...
    for (let i: number = 0; i < shortcutKeys.length; i++) {
      let annotation: ShapeAnnotationModel = {
        content:
          shortcutKeys[i].key.toString() +
          ": " +
          shortcutKeys[i].value.toString(),
        offset: { x: 0.1, y: y },
        visibility: true,
        style: { color: "white" },
...
export abstract class MindMapUtilityMethods {
  public static mindmapPaste(): void {
    let diagram: Diagram = this.selectedItem.selectedDiagram as Diagram;
    let selectedNode: Node = (diagram.selectedItems as any).nodes[0] as Node;
    ...
    if (this.selectedItem.pasteData.length > 0) {
      ...
      if (selectedNode.id !== "rootNode") {
        selecteditemOrientation = (
          selectedNode.addInfo as { [key: string]: Object }
        ).orientation.toString();
      } else {
        selecteditemOrientation = (
          (this.selectedItem.pastedFirstItem as Node).addInfo as {
            [key: string]: Object;
          }
        ).orientation.toString();
      }
      ...
      let connector: ConnectorModel = MindMapUtilityMethods.setConnectorDefault(
        diagram,
        selecteditemOrientation,
        mindmapData.connector,
        selectedNode.id,
        selectedelement.id
      );
  ...
  private static updateLevel(
    ...
    for (let i: number = parentNode.outEdges.length - 1; i >= 0; i--) {
      ...
      (childNode.addInfo as { [key: string]: Object }).orientation =
        orientation;
      ...
      if (childNode.outEdges.length > 0) {
        this.updateLevel(
          childNode,
          (parentNode.addInfo as any).level,
          orientation
        );
      } else {
        (childNode.addInfo as INodeInfo).level =
          (parentNode.addInfo as any).level + 1;
  ...
  public static addMindMapLevels(level: string): void {
    ...
    let dropdowndatasource: any = dropdownlist.dataSource as {
      [key: string]: Object;
    }[];
  ...
  public static createEmptyMindMap(): NodeModel {
    let node: NodeModel = {
      ...
      minHeight: 50,
      annotations: [
        { content: "MindMap", style: { color: "#000000", fontSize: 12 } },
      ],
      ...
      addInfo: { level: 0 },
      style: { fill: "#D0ECFF", strokeColor: "#80BFEA", strokeWidth: 1 },
    ...
    let node1: NodeModel = {
      ...
      height: 450,
      offsetX:
        (this.selectedItem as any).selectedDiagram.scrollSettings
          .viewPortWidth - 200,
      offsetY: 230,
    ...
    (this.selectedItem as any).selectedDiagram.add(node1);
    (
  ...
  public static onHideNodeClick(): void {
    let node1: NodeModel = MindMapUtilityMethods.getNode(
      (this.selectedItem as any).selectedDiagram.nodes,
      "textNode"
    );
  ...
  public static getShortCutString(): string {
    return (
      '<div style="width: 400px; height: 450px; padding: 10px; background-color: #FFF7B5; border: 1px solid #FFF7B5">' +
  ...
  public static getMindMapShape(parentNode: NodeModel): {
    [key: string]: Object;
  } {
    let sss: { [key: string]: Object } = {};
    let node: NodeModel = {};
    let connector: ConnectorModel = {};
    let addInfo: { [key: string]: Object } = parentNode.addInfo as {
      [key: string]: Object;
    };
    ...
    if (addInfo.level < 1) {
      (node as any).style.fill =
        this.selectedItem.utilityMethods.fillColorCode[this.lastFillIndex];
      (node as any).style.strokeColor =
        this.selectedItem.utilityMethods.borderColorCode[this.lastFillIndex];
      if (
        this.lastFillIndex + 1 >=
        this.selectedItem.utilityMethods.fillColorCode.length
      ) {
      ...
      (node as any).style.strokeColor = (node as any).style.fill = (
        parentNode as any
      ).style.fill;
    }
    (connector as any).style.strokeColor = (node as any).style.fill;
  ...
  public static addNode(orientation: string): void {
    ...
    let selectedNode: Node = (diagram.selectedItems as any).nodes[0] as Node;
    if (selectedNode.id !== "rootNode") {
      let selectedNodeOrientation: string = (
        selectedNode.addInfo as { [key: string]: Object }
      ).orientation.toString();
    ...
    diagram.startGroupAction();
    let mindmapData: { [key: string]: Object } =
      this.getMindMapShape(selectedNode);
    let node: NodeModel = mindmapData.node;
    this.addMindMapLevels(
    ...
    diagram.add(node);
    let connector: ConnectorModel = this.setConnectorDefault(
      diagram,
      orientation,
      mindmapData.connector,
      selectedNode.id,
  ...
  public static addSibilingChild(position: string): void {
    ...
    if (selectedNode.id !== "rootNode") {
      ...
      diagram.startGroupAction();
      let mindmapData: { [key: string]: Object } = this.getMindMapShape(
        this.getNode(diagram.nodes, connector1.sourceID)
      );
      let node: NodeModel = mindmapData.node;
      node.id = "node" + this.selectedItem.randomIdGenerator();
      if (node.addInfo) {
  ...
  public static getConnector(
    ...
    return null as unknown as Connector;
  }
  ...
  public static setConnectorDefault(
    diagram: Diagram,
    orientation: string,
    ...
    (connector as any).style.strokeWidth = 3;
    return connector;
  }
