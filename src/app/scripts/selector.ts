...
export class NodeProperties {
  ...
  public triggerPropertyChange(
    ...
    if (!isNullOrUndefined(this.propertyChange as any)) {
      (this.propertyChange as any).call(this, {
    ...
    if (!this.preventPropertyChange) {
      ...
      if (diagram) {
        if ((diagram as any).selectedItems.nodes.length > 0) {
          let selectedNodes: NodeModel[] = (this.selectedDiagram as any)
            .selectedItems.nodes;
          for (let i: number = 0; i < (selectedNodes as any).length; i++) {
            ...
            switch (propertyName1) {
              case "offsetx":
                node.offsetX = (
                  document.getElementById("nodeOffsetX") as any
                ).ej2_instances[0].value;
                break;
              case "offsety":
                node.offsetY = (
                  document.getElementById("nodeOffsetY") as any
                ).ej2_instances[0].value;
                break;
              case "width":
                node.width = (
                  document.getElementById("nodeWidth") as any
                ).ej2_instances[0].value;
                break;
              case "height":
                node.height = (
                  document.getElementById("nodeHeight") as any
                ).ej2_instances[0].value;
                break;
              case "rotateangle":
        ...
        if (diagram.connectors.length > 0) {
          let selectedNodes: ConnectorModel[] | undefined =
          ...
          switch (args.propertyName.toString().toLowerCase()) {
            ...
            case "linewidth":
              connector.style.strokeWidth = (
                document.getElementById("lineWidth") as any
              ).ej2_instances[0].value;
            ...
            case "opacity":
              ...
              (connector as any).targetDecorator.style.opacity =
                connector.style.opacity;
              (connector as any).sourceDecorator.style.opacity =
                connector.style.opacity;
  ...
  public textPropertyChange(args: { [key: string]: Object }): void {
    if (!this.preventPropertyChange) {
      ...
      if (diagram) {
        ...
        selectedObjects = (selectedObjects as any).concat(
          diagram.selectedItems.connectors
        );
            ...
            if (node instanceof Node || node instanceof Connector) {
              if (node.annotations.length > 0) {
                for (let j: number = 0; j < node.annotations.length; j++) {
                  let annotation: TextStyleModel | undefined =
                    node.annotations[j].style;
    ...
    if (node.addInfo && node.annotations.length > 0) {
      ...
      switch (propertyName) {
        ...
        case "fontsize":
          ...
          break;
        case "fontcolor":
          (annotation as any).color = this.getColor(
            this.mindmapSettings.fontColor
          );
          break;
        case "textopacity":
