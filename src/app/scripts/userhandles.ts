...
export class CustomTool {
  ...
  public getTool(action: string): ToolBase {
    ...
    if (action === "leftHandle") {
      let leftTool: LeftExtendTool = new LeftExtendTool(
        (this.selectedItem.selectedDiagram as any).commandHandler
      );
      leftTool.selectedItem = this.selectedItem;
      return leftTool;
    } else if (action === "rightHandle") {
      let rightTool: RightExtendTool = new RightExtendTool(
        (this.selectedItem.selectedDiagram as any).commandHandler
      );
      rightTool.selectedItem = this.selectedItem;
      return rightTool;
    } else if (action === "removeHandle") {
      let removeTool: RemoveTool = new RemoveTool(
        (this.selectedItem.selectedDiagram as any).commandHandler
      );
      removeTool.selectedItem = this.selectedItem;
      return removeTool;
    } else if (action === "orgAddHandle") {
      let orgAddTool: OrgAddHandleTool = new OrgAddHandleTool(
        (this.selectedItem.selectedDiagram as any).commandHandler
      );
      orgAddTool.selectedItem = this.selectedItem;
      return orgAddTool;
    } else if (action === "orgRemoveHandle") {
      let orgRemoveTool: OrgRemoveHandleTool = new OrgRemoveHandleTool(
        (this.selectedItem.selectedDiagram as any).commandHandler
      );
...
class OrgAddHandleTool extends ToolBase {
  ...
  public mouseUp(args: MouseEventArgs): void {
    if (this.inAction) {
      ...
      if (selectedObject[0]) {
        if (selectedObject[0] instanceof Node) {
          OrgChartUtilityMethods.addChild(
            this.selectedItem.selectedDiagram.selectedItems.nodes[0].id
          );
        }
      }
    }
