...
export class DiagramTheme {
  ...
  public getShapeType(shapeType: FlowShapes | BasicShapes): string {
    if (shapeType === "Ellipse" || shapeType === "Terminator") {
      ...
      shapeType === "SequentialData" ||
      shapeType === "Sort" ||
      shapeType === "MultiDocument" ||
      shapeType === "Collate" ||
      shapeType === "Or" ||
      shapeType === "InternalStorage" ||
      shapeType === "SequentialAccessStorage" ||
      shapeType === "Annotation2" ||
  ...
  public themeMouseOver(args: MouseEvent): void {
    ...
    if (target.className === "db-theme-style-div") {
      let themeName: string = target.children[0].className.replace(
        "db-theme-style ",
        ""
      );
  ...
  public applyOldStyle(): void {
    ...
    for (let i: number = 0; i < this.nodeOldStyles.length; i++) {
      ...
      let node: Node = MindMapUtilityMethods.getNode(
        (diagram as any).nodes,
        this.nodeOldStyles[i].name.toString()
      );
      ...
      if (node.annotations.length > 0) {
        if (!node.annotations[0].hyperlink) {
          (node as any).annotations[0].style.color = themeStyle.fontColor;
        }
      }
    ...
    for (let i: number = 0; i < this.connectorOldStyles.length; i++) {
      ...
      let connector: Connector = MindMapUtilityMethods.getConnector(
        ...
        this.connectorOldStyles[i].name.toString()
      );
      connector.style.strokeColor = themeStyle.border.toString();
      (connector as any).sourceDecorator.style.fill = (
        connector as any
      ).sourceDecorator.style.strokeColor = themeStyle.border.toString();
      (connector as any).targetDecorator.style.fill = (
        connector as any
      ).targetDecorator.style.strokeColor = themeStyle.border.toString();
  ...
  public themeClick(args: MouseEvent): void {
    ...
    if (target.classList.contains("db-theme-style-div")) {
      ...
      for (let i: number = 0; i < this.colorList.length; i++) {
        let element: HTMLDivElement = document.getElementsByClassName(
          String(this.colorList[i].themeName)
  ...
  public setNodeOldStyles(): void {
    ...
    let diagram: Diagram | undefined = this.selectedItem.selectedDiagram;
    if ((diagram as any).nodes.length > 0) {
      let nodes: Node[] = (diagram as any).nodes as Node[];
      for (let i: number = 0; i < nodes.length; i++) {
        ...
        if (node.annotations.length > 0) {
          if (!node.annotations[0].hyperlink) {
            (style as any).fontColor = (node as any).annotations[0].style.color;
          }
        }
        this.nodeOldStyles.push(style as any);
      }
    }
    if ((diagram as any).connectors.length > 0) {
      let connectors: Connector[] = (diagram as any).connectors as Connector[];
      for (let i: number = 0; i < connectors.length; i++) {
        let connector: Connector = connectors[i];
        (this.connectorOldStyles as any).push({
          name: connector.id,
  ...
  public applyStyle(themeName: string): void {
    ...
    if ((diagram as any).connectors.length > 0 && themeType) {
      let connectors: Connector[] = (diagram as any).connectors as Connector[];
      for (let i: number = 0; i < connectors.length; i++) {
        let connector: Connector = connectors[i];
  ...
  public applyThemeStyleforElement(
    element: object,
    themeName?: { [key: string]: any }
  ): void {
    ...
    if (element instanceof Node) {
      ...
      if (node.shape) {
        ...
        if (node.shape.type === "Flow" || node.shape.type === "Basic") {
          let shapeModel: FlowShapeModel | BasicShapeModel = node.shape as
            | FlowShapeModel
            | BasicShapeModel;
          shapeStyle = this.getShapeStyle(
            this.getShapeType((shapeModel as any).shape),
            themeType.themeStyle as { [key: string]: string }[]
          );
          if (shapeStyle) {
            ...
            if (node.annotations.length > 0) {
              if (
                !(
                  node.annotations[0].hyperlink &&
                  node.annotations[0].hyperlink.link
                )
              ) {
                (node as any).annotations[0].style.color = shapeStyle.textColor;
              }
            }
            (diagram as any).dataBind();
          }
      ...
      (connector as any).sourceDecorator.style.fill = (
        connector as any
      ).sourceDecorator.style.strokeColor = themeType.lineColor.toString();
      (connector as any).targetDecorator.style.fill = (
        connector as any
      ).targetDecorator.style.strokeColor = themeType.lineColor.toString();
