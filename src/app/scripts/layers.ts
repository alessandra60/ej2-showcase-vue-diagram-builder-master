...
export class DiagramBuilderLayer {
  ...
  private getLayers(): LayerModel[] {
    return (this.selectedItem as any).selectedDiagram.layers.sort(
      (a: LayerModel, b: LayerModel): number => {
  ...
  public getLayerDialogContent(): void {
    ...
    if (layers.length > 0) {
      ...
      for (let i: number = layers.length - 1; i >= 0; i--) {
        ...
        layerDialogContent.appendChild(
          this.cloneLayerTemplate(layers[i], orderType, i)
        );
      }
      if (layers.length === 1) {
  ...
  public cloneLayerTemplate(
    layer: LayerModel,
    orderType: string,
    index: number
  ): HTMLDivElement {
    let layerTemplate: HTMLDivElement = (
      document.getElementsByClassName("db-layer-template")[0] as HTMLDivElement
    ...
    let layerNameElement: HTMLSpanElement =
      layerTemplate.getElementsByClassName(
        "db-layer-name"
      )[0] as HTMLSpanElement;
    layerNameElement.innerHTML = (layer.addInfo as { [key: string]: Object })
      .name as string;
    layerNameElement.className = "db-layer-name " + layer.id;

    (layerNameElement.parentNode as HTMLDivElement).style.width =
      "calc(100% - " + 88 + "px)";
  ...
  public triggerEvents(): void {
    ...
    for (let i: number = 0; i < layers.length; i++) {
      ...
      let visibleElement: HTMLButtonElement =
        visibleElements[layers.length - i];
      ...
      if (!layer.visible) {
        (visibleElement.parentElement as any).className =
          "db-layer-content-btn db-layer-invisible";
      }
      ...
      if (layer.lock) {
        (lockElement.parentElement as any).className =
          "db-layer-content-btn db-layer-invisible";
      }
    }
    ...
    for (let i: number = 0; i < layerNameElements.length; i++) {
      ...
      (layerNameElement.parentElement as any).onclick =
        this.setActiveLayer.bind(this);
      (layerNameElement.parentElement as any).ondblclick =
        this.btnRenameLayer.bind(this);
      (
        (layerNameElement.parentElement as any).children[1] as HTMLInputElement
      ).addEventListener("focusout", (this.renameLayer as any).bind(this));
      (
        (layerNameElement.parentElement as any).children[1] as HTMLInputElement
      ).addEventListener(
        "keydown",
        (this.renameLayerKeyDown as any).bind(this)
      );
    }
  }
  ...
  public initLayerBottomPanel(): void {
    ...
    (document.getElementById("btnRemove") as any).onclick =
      this.btnRemoveLayer.bind(this);
    ...
    (document.getElementById("btnDuplicate") as any).onclick =
      this.btnDuplicateLayer.bind(this);

    let addLayer: Button = new Button({ iconCss: "sf-icon-Plus" });
    addLayer.appendTo("#btnAdd");
    (document.getElementById("btnAdd") as any).onclick =
      this.btnAddLayer.bind(this);

    let closeLayer: Button = new Button({ iconCss: "sf-icon-Close" });
    closeLayer.appendTo("#btnCloseLayer");
    (document.getElementById("btnCloseLayer") as any).onclick =
      this.btnCloseDialog.bind(this);
  }

  public changeLayerSelection(args: MouseEvent): void {
    let element: any = args.target;
    let layerName: string = element.className
      .replace("db-layer-lock e-control e-btn e-lib ", "")
      .replace(" e-icon-btn", "")
      .replace(" e-ripple", "");
    let layer: LayerModel = this.findLayer(layerName);
    layer.lock = !layer.lock;
    element.ej2_instances[0].iconCss = layer.lock
      ? "sf-icon-Lock"
      : "sf-icon-Unlock";
    element.title = layer.lock ? "Lock" : "Unlock";
    element.parentElement.className = layer.lock
      ? "db-layer-content-btn db-layer-invisible"
  ...
  public changeLayerVisibility(args: MouseEvent): void {
    let element: any = args.target;
    let layerName: string = element.className
      .replace("db-layer-visible e-control e-btn e-lib ", "")
      .replace(" e-icon-btn", "")
      .replace(" e-ripple", "");
    let layer: LayerModel = this.findLayer(layerName);
    layer.visible = !layer.visible;
    element.ej2_instances[0].iconCss = layer.visible
      ? "sf-icon-View"
      : "sf-icon-Invisible";
    element.title = layer.visible ? "Visible" : "Invisible";
    element.parentElement.className = layer.visible
      ? "db-layer-content-btn"
      : "db-layer-content-btn db-layer-invisible";
    (this.selectedItem as any).selectedDiagram.dataBind();
  }
  ...
  public changeLayerZorder(args: MouseEvent): void {
    ...
    let layerName: string = element.className
      .replace("db-layer-order-second e-control e-btn ", "")
      .replace(" e-icon-btn", "")
      .replace(" e-ripple", "");
    layerName = layerName
      .replace("db-layer-order-first e-control e-btn ", "")
      .replace(" e-ripple", "");
  ...
  public setActiveLayer(args: MouseEvent): void {
    if (!this.isEditing) {
      ...
      (this.selectedItem as any).selectedDiagram.setActiveLayer(
        target.children[0].className.replace("db-layer-name ", "")
      );
      let elements: HTMLCollectionOf<HTMLDivElement> =
        document.getElementsByClassName(
      ...
      (target.parentElement as any).parentElement.className =
        "db-layer-template active";
    }
  }
  ...
  public btnRenameLayer(args: MouseEvent): void {
    if (!this.isEditing) {
      ...
      let inputElement: HTMLInputElement = target
  ...
  public renameLayer(args: MouseEvent): void {
    ...
    let addInfo: { [key: string]: Object } = (this.selectedItem as any)
      .selectedDiagram.activeLayer.addInfo as { [key: string]: Object };
    (target.parentElement as any).children[0].innerHTML = addInfo.name =
      target.value;
  ...
  public btnRemoveLayer(args: MouseEvent): void {
    let activeLayerIndex: number = this.getLayers().indexOf(
      (this.selectedItem as any).selectedDiagram.activeLayer
    );
    (this.selectedItem as any).selectedDiagram.removeLayer(
      (this.selectedItem as any).selectedDiagram.activeLayer.id
    );
    if (activeLayerIndex - 1 < 0) {
      (this.selectedItem as any).selectedDiagram.setActiveLayer(
        (this.getLayers()[0] as any).id
      );
    } else {
      (this.selectedItem as any).selectedDiagram.setActiveLayer(
        (this.getLayers()[activeLayerIndex - 1] as any).id
      );
    }
    this.getLayerDialogContent();
  }
  ...
  public btnDuplicateLayer(): void {
    let name: string = (
      (this.selectedItem as any).selectedDiagram.activeLayer.addInfo as {
        [key: string]: string;
      }
    ).name;
    (this.selectedItem as any).selectedDiagram.cloneLayer(
      (this.selectedItem as any).selectedDiagram.activeLayer.id
    );
    (this.selectedItem as any).selectedDiagram.layers[
      (this.selectedItem as any).selectedDiagram.layers.length - 1
    ].addInfo = {
      name: name + " Copy",
    };
    this.getLayerDialogContent();
  ...
  public getActiveLayer(layer: LayerModel): LayerModel {
    if (
      layer.id === (this.selectedItem as any).selectedDiagram.activeLayer.id
    ) {
      return layer;
    }
    return {};
