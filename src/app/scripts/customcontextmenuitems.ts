...
export class CustomContextMenuItems {
  ...
  public getHiddenMenuItems(diagram: Diagram): string[] {
    ...
    if (
      (diagram.selectedItems.nodes as any).length > 0 ||
      (diagram.selectedItems.connectors as any).length > 0
    ) {
      hiddenItems.splice(hiddenItems.indexOf("duplicate"), 1);
    }
    if (
      (diagram.selectedItems.nodes as any).length === 1 &&
      (diagram.selectedItems.connectors as any).length === 0
    ) {
  ...
  public updateBpmnShape(diagram: Diagram, item: MenuItemModel): void {
    ...
    // } else if (diagram.selectedItems.nodes.length === 1 && (diagram.selectedItems.connectors as any).length === 0) {
