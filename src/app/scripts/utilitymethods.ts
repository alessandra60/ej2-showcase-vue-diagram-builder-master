...
export class UtilityMethods {
  ...
  public bindNodeProperties(
    ...
    selectedItem.nodeProperties.offsetX =
      Math.round((node as any).offsetX * 100) / 100;
    selectedItem.nodeProperties.offsetY =
      Math.round((node as any).offsetY * 100) / 100;
    selectedItem.nodeProperties.width = node.width
      ? Math.round(node.width * 100) / 100
      : Math.round((node as any).minWidth * 100) / 100;
    selectedItem.nodeProperties.height = node.height
      ? Math.round(node.height * 100) / 100
      : Math.round((node as any).minHeight * 100) / 100;
    ...
    selectedItem.nodeProperties.strokeStyle = (node as any).style
      .strokeDashArray
      ? (node as any).style.strokeDashArray
      : "None";
    selectedItem.nodeProperties.strokeWidth = (node as any).style.strokeWidth;

    selectedItem.nodeProperties.fillColor = this.getHexColor(
    ...
    selectedItem.nodeProperties.aspectRatio =
      (node as any).constraints & NodeConstraints.AspectRatio ? true : false;
    selectedItem.nodeProperties.gradient =
      (node as any).style.gradient.type !== "None" ? true : false;
    let gradientElement: HTMLElement | null =
      document.getElementById("gradientStyle");
    if (selectedItem.nodeProperties.gradient) {
      (gradientElement as HTMLElement).className =
        "row db-prop-row db-gradient-style-show";
      selectedItem.nodeProperties.gradientColor = (
        node as any
      ).style.gradient.stops[1].color;
      let gradient: LinearGradient = (node as any).style
      ...
      (gradientElement as HTMLElement).className =
        "row db-prop-row db-gradient-style-hide";
  ...
  public bindMindMapProperties(
    ...
    selectedItem.mindmapSettings.strokeStyle = (node as any).style
      .strokeDashArray
      ? (node as any).style.strokeDashArray
      : "None";
    ...
    selectedItem.mindmapSettings.opacity =
      ((node as any).style.opacity || 1) * 100;
    selectedItem.mindmapSettings.opacityText =
      (selectedItem.mindmapSettings.opacity || "100") + "%";
    if ((node as any).annotations.length > 0) {
      let annotation: TextStyle = (node as any).annotations[0]
        .style as TextStyle;
  ...
  public bindTextProperties(
    ...
    selectedItem.preventPropertyChange = true;
    selectedItem.textProperties.fontColor = this.getHexColor(
      (text as any).color
    );
  ...
  public bindConnectorProperties(
    ...
    selectedItem.connectorProperties.lineColor = this.getHexColor(
      (connector as any).style.strokeColor
    );
    selectedItem.connectorProperties.lineStyle = (connector as any).style
      .strokeDashArray
      ? (connector as any).style.strokeDashArray
      : "None";
    (selectedItem as any).connectorProperties.lineType = connector.type;
    selectedItem.connectorProperties.lineWidth = (
      connector as any
    ).style.strokeWidth;
    selectedItem.connectorProperties.sourceType = (
      connector as any
    ).sourceDecorator.shape;
    selectedItem.connectorProperties.targetType = (
      connector as any
    ).targetDecorator.shape;
    selectedItem.connectorProperties.opacity =
      (connector as any).style.opacity * 100;
    selectedItem.connectorProperties.opacityText =
      selectedItem.connectorProperties.opacity + "%";
    (selectedItem as any).connectorProperties.lineJumpSize =
      connector.bridgeSpace;
    selectedItem.connectorProperties.lineJump =
      (connector as any).constraints & ConnectorConstraints.Bridging
        ? true
        : false;
    ...
    (selectedItem as any).connectorProperties.targetSize = (
      connector as any
    ).targetDecorator.width;
    (selectedItem as any).connectorProperties.sourceSize = (
      connector as any
    ).sourceDecorator.width;
    selectedItem.preventPropertyChange = false;
  }
  ...
  public getHexColor(colorStr: string): string {
    ...
    let colors: number[] = (
      window.getComputedStyle(document.body.appendChild(a)) as any
    ).color
      .match(/\d+/g)
      .map((a: string): number => {
  ...
  public objectTypeChange(objectType: string): void {
    ...
    (document.getElementById("textPropertyContainer") as any).style.display =
      "none";
    (
      document.getElementById("connectorPropertyContainer") as any
    ).style.display = "none";
    switch (objectType) {
      case "diagram":
        (
          document.getElementById("diagramPropertyContainer") as any
        ).style.display = "";
        break;
      case "node":
        (
          document.getElementById("nodePropertyContainer") as any
        ).style.display = "";
        break;
      case "connector":
        (
    ...
    ajax.onSuccess = (data: string): void => {
      ...
      if (selectedItem.diagramType === "MindMap") {
        let rootNode: Node = MindMapUtilityMethods.getNode(
          (selectedItem.selectedDiagram as any).nodes,
          "rootNode"
        );
      ...
      if (selectedItem.diagramType === "OrgChart") {
        (selectedItem.selectedDiagram as any).layout.getLayoutInfo =
          OrgChartUtilityMethods.getLayoutInfo.bind(OrgChartUtilityMethods);
        (selectedItem.selectedDiagram as any).selectedItems.userHandles =
          OrgChartUtilityMethods.handle;
        (selectedItem.selectedDiagram as any).selectedItems.constraints =
  ...
  public generateDiagram(
    ...
    let diagramName: string = (target.parentElement as any).children[1]
  ...
  public enableMenuItems(
    ...
    let selectedItems: Object[] | undefined =
      selectedItem.selectedDiagram.selectedItems.nodes;
    selectedItems = (selectedItems as any).concat(
    ...
    if (itemText) {
      ...
      if (
        itemText === "Undo" &&
        (selectedItem as any).selectedDiagram.historyManager.undoStack
          .length === 0
      ) {
        return true;
      }
      if (
        itemText === "Redo" &&
        (selectedItem as any).selectedDiagram.historyManager.redoStack
          .length === 0
      ) {
        return true;
      }
      if (itemText === "Select All") {
        if (
          selectedItem.diagramType !== "GeneralDiagram" ||
          ((selectedItem.selectedDiagram as any).nodes.length === 0 &&
            (selectedItem.selectedDiagram as any).connectors.length === 0)
        ) {
          return true;
        }
      }
  ...
  public enableArrangeMenuItems(selectedItem: SelectorViewModel): void {
    ...
    let selectedItems: Object[] | undefined = (
      selectedItem.selectedDiagram as any
    ).selectedItems.nodes;
    selectedItems = (selectedItems as any).concat(
      (selectedItem.selectedDiagram as any).selectedItems.connectors
    );
  ...
  public updateLayout(
    ...
    for (
      let i: number = 0;
      i < (selectedItem.selectedDiagram as any).nodes.length;
      i++
    ) {
      ...
      if (node.id !== "textNode") {
        ...
        (selectedItem.selectedDiagram as any).removeLabels(
          node,
          node.annotations
        );
        propName = "Image URL";
