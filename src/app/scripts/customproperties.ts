...
export class CustomProperties {
  ...
  public getPropertyDialogContent(addInfo: Object): void {
    ...
    if (addInfo) {
      let addInfo1: { [key: string]: Object } = addInfo as {
        [key: string]: Object;
      };
      let keys: string[] = Object.keys(addInfo1);
      for (let i: number = 0; i < keys.length; i++) {
        propertyDialogContent.appendChild(
          this.clonePropInfoTemplate(
            keys[i],
            addInfo1[keys[i]] as { [key: string]: Object }
          )
        );
      }
    ...
    (this.customPropertyDialog as any).content =
      propertyDialogContent.outerHTML;
  ...
  private triggerEvents(addInfo: Object): void {
    ...
    let keys: string[] = Object.keys(addInfo1);
    for (let i: number = 0; i < keys.length; i++) {
      let removeBtnElement: HTMLButtonElement = removeBtnElements[i + 1]
      ...
      let checkboxTooltipElement: HTMLInputElement = removeCheckBoxElements[
        i + 1
      ].children[0] as HTMLInputElement;
      let checkboxTooltip: CheckBox = new CheckBox({
        checked: Boolean(
          (addInfo1[keys[i]] as { [key: string]: Object }).checked
        ),
        cssClass: keys[i],
      });
      ...
      (propertyValueElements[i + 1].children[0] as HTMLInputElement).value = (
        addInfo1[keys[i]] as { [key: string]: Object }
      ).value.toString();
      (propertyValueElements[i + 1].children[0] as any).onchange =
        this.valueChange.bind(this);
    }
    let propButton: HTMLButtonElement = document.getElementsByClassName(
      "db-custom-prop-button"
  ...
  private clonePropInfoTemplate(
    ...
    propertyInfo.getElementsByClassName("propertyNameDiv")[0].innerHTML =
      propertyName;

    let removeBtnElement: HTMLButtonElement =
      propertyInfo.getElementsByClassName(
        "btnRemoveProperty"
  ...
  private valueChange(args: MouseEvent): void {
    ...
    let addInfo: any = (
      (this.selectedItem as any).selectedDiagram.selectedItems as any
    ).nodes[0].addInfo;
    addInfo[
      (target as any).parentElement.parentElement.children[0].innerHTML
    ].value = target.value;
  ...
  private removeField(args: ChangeEventArgs): void {
    let target: HTMLInputElement = (args.event as any)
      .target as HTMLInputElement;
    let className: string = (
      target.parentElement as any
    ).parentElement.className
      .replace("e-checkbox-wrapper ", "")
      .trim();
    for (
      let i: number = 0;
  ...
  private showConfirmationDialog(args: MouseEvent): void {
    let target: any = args.target as HTMLElement;
    if (target.tagName.toLowerCase() === "span") {
      target = target.parentElement;
    }
    this.deleteField = target.className
      .replace("btnRemoveProperty e-control e-btn ", "")
      .replace(" e-icon-btn", "")
      .trim();
  ...
  public removeProperty(args: MouseEvent): void {
    ...
    let addInfo: any = ((this.selectedItem as any).selectedDiagram as any)
      .selectedItems.nodes[0].addInfo;
    this.getPropertyDialogContent(addInfo);
    let imageField: boolean = false;
    if (addInfo["Image URL"] && addInfo["Image URL"].checked) {
      imageField = true;
    }
    (this.selectedItem as any).utilityMethods.updateLayout(
  ...
  private addCustomProperty(): void {
    let propName: string = (
      document.getElementsByClassName("txtPropertyName")[1] as HTMLInputElement
    ).value;
    if (propName) {
      for (
        let i: number = 0;
      ...
      this.getPropertyDialogContent(
        (this.selectedItem.selectedDiagram as any).selectedItems.nodes[0]
          .addInfo
      );
    } else {
      alert("Invalid Name");
  ...
  public setTooltip(node: NodeModel, content: string): void {
    if (content) {
      node.constraints = (node.constraints as any) | NodeConstraints.Tooltip;
      node.tooltip = {
        content: content,
