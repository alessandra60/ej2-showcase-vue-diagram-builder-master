...
export class DiagramClientSideEvents {
    ...
    if (args.state === "Changed") {
      if (this.selectedItem.diagramType === "MindMap") {
        if (
          ...
          (diagram as any).selectedItems.nodes.length === 1
        ) {
        ...
        let selectedItems: Object[] = (this.selectedItem as any).selectedDiagram
          .selectedItems.nodes;
        selectedItems = selectedItems.concat(
          (this.selectedItem as any).selectedDiagram.selectedItems.connectors
        );
        this.selectedItem.utilityMethods.enableToolbarItems(selectedItems);
        let nodeContainer: HTMLElement | null = document.getElementById(
  ...
  private multipleSelectionSettings(selectedItems: Object[]): void {
    ...
    let selectItem1: SelectorModel = (this.selectedItem as any).selectedDiagram
      .selectedItems;
    if (showNodePanel) {
    ...
    if (showConnectorPanel && !showNodePanel) {
      ...
      this.selectedItem.utilityMethods.bindConnectorProperties(
        (selectItem1 as any).connectors[0],
        this.selectedItem
      );
    }
    if (showTextPanel || showConTextPanel) {
      ...
      if (showTextPanel && showConTextPanel) {
        (document.getElementById("textPositionDiv") as any).style.display =
          "none";
        (document.getElementById("textColorDiv") as any).className =
        ...
        if (showConTextPanel) {
          (this.ddlTextPosition as any).dataSource =
  ...
  private singleSelectionSettings(selectedObject: Object): void {
    ...
    if (object.shape && object.shape.type === "Text") {
      (document.getElementById("textPropertyContainer") as any).style.display =
        "";
      (
        document.getElementById("toolbarTextAlignmentDiv") as any
      ).style.display = "none";
      (document.getElementById("textPositionDiv") as any).style.display =
        "none";
      (document.getElementById("textColorDiv") as any).className =
      ...
      (document.getElementById("textColorDiv") as any).className =
        "col-xs-6 db-col-right";
      this.selectedItem.utilityMethods.bindTextProperties(
        (object as any).annotations[0].style,
        this.selectedItem
      );
      this.selectedItem.utilityMethods.updateHorVertAlign(
        (object as any).annotations[0].horizontalAlignment,
        (object as any).annotations[0].verticalAlignment
      );
      if (object.annotations[0] instanceof ShapeAnnotation) {
        annotation = object.annotations[0] as ShapeAnnotation;
        (this.ddlTextPosition as any).dataSource =
          this.selectedItem.textProperties.getNodeTextPositions();
        (this.ddlTextPosition as any).value = (
          this.selectedItem as any
        ).textProperties.textPosition = null;
        (this.ddlTextPosition as any).dataBind();
        (this.ddlTextPosition as any).value =
  ...
  public nodePositionChange(args: IDraggingEventArgs): void {
    ...
    this.selectedItem.nodeProperties.offsetX =
      Math.round((args as any).newValue.offsetX * 100) / 100;
    this.selectedItem.nodeProperties.offsetY =
      Math.round((args as any).newValue.offsetY * 100) / 100;
  ...
  public nodeSizeChange(args: ISizeChangeEventArgs): void {
    ...
    this.selectedItem.nodeProperties.width =
      Math.round((args as any).newValue.width * 100) / 100;
    this.selectedItem.nodeProperties.height =
      Math.round((args as any).newValue.height * 100) / 100;
    if (args.state === "Completed") {
      this.selectedItem.isModified = true;
      this.selectedItem.preventPropertyChange = false;
    }
  ...
  public nodeRotationChange(args: IRotationEventArgs): void {
    ...
    this.selectedItem.nodeProperties.rotateAngle =
      Math.round((args as any).newValue.rotateAngle * 100) / 100;
    this.selectedItem.preventPropertyChange = false;
    if (args.state === "Completed") {
      this.selectedItem.isModified = true;
    }
  ...
  public keyDown(args: IKeyEventArgs) {
    if (this.selectedItem.diagramType === "MindMap") {
      ...
      if (
        args.key === "Enter" &&
        args.keyModifiers === 0 &&
        diagram.diagramActions & DiagramAction.TextEdit
      ) {
        diagram.endEdit();
      }
    }
...
export class DiagramPropertyBinding {
  ...
  public pageBreaksChange(args: CheckBoxChangeEventArgs): void {
    if (args.event) {
      ...
      (this.selectedItem as any).selectedDiagram.pageSettings.showPageBreaks =
        args.checked;
    }
  }
  ...
  public pageOrientationChange(args: ButtonChangeArgs): void {
    if (args.event) {
      ...
      diagram.dataBind();
      (this.selectedItem as any).pageSettings.pageWidth =
        diagram.pageSettings.width;
      (this.selectedItem as any).pageSettings.pageHeight =
        diagram.pageSettings.height;
    }
  }
  ...
  public toolbarTextSubAlignChange(args: ToolbarClickEventArgs): void {
    let propertyName: string = (args as any).item.tooltipText.replace(
      /[" "]/g,
      ""
    );
  ...
  public textPropertyChange(propertyName: string, propertyValue: Object): void {
    if (!this.selectedItem.preventPropertyChange) {
      ...
      selectedObjects = selectedObjects.concat(
        (diagram as any).selectedItems.connectors
      );
            ...
            if (node.annotations.length > 0) {
              for (let j: number = 0; j < node.annotations.length; j++) {
                let annotation: ShapeAnnotationModel | PathAnnotationModel =
                  null as unknown as ShapeAnnotationModel;
                ...
                if (
                  ...
                  this.selectedItem.utilityMethods.updateHorVertAlign(
                    annotation.horizontalAlignment,
                    (annotation as any).verticalAlignment
                  );
                } else if (
                  propertyName === "top" ||
                  propertyName === "bottom"
                  ...
                  this.selectedItem.utilityMethods.updateHorVertAlign(
                    (annotation as any).horizontalAlignment,
                    annotation.verticalAlignment
                  );
...
export class MindMapPropertyBinding {
  ...
  public mindmapTextStyleChange(args: ToolbarClickEventArgs): void {
    this.updateMindMapTextStyle(
      (args as any).item.tooltipText.toLowerCase(),
      false
    );
  }
    ...
    if (diagram.nodes.length > 0) {
      for (let i: number = 0; i < diagram.nodes.length; i++) {
        ...
        if (node.addInfo && node.annotations.length > 0) {
          let annotation: TextStyleModel = node.annotations[0]
  ...
  public mindmapPatternChange(args: MouseEvent): void {
    ...
    for (
      let i: number = 0;
      i < (this.selectedItem as any).selectedDiagram.nodes.length;
      i++
    ) {
      let node: Node = (this.selectedItem as any).selectedDiagram.nodes[
        i
      ] as Node;
      if (node.id !== "textNode") {
    ...
    for (
      let i: number = 0;
      i < (this.selectedItem as any).selectedDiagram.connectors.length;
      i++
    ) {
      let connector: Connector = (this.selectedItem as any).selectedDiagram
...
export class OrgChartPropertyBinding {
  ...
  public orgChartSpacingChange(args: NumericChangeEventArgs): void {
    if (args.event) {
      ...
      if (target.id === "orgHorizontalSpacing") {
        (this.selectedItem as any).selectedDiagram.layout.horizontalSpacing =
          args.value;
      } else {
        (this.selectedItem as any).selectedDiagram.layout.verticalSpacing =
          args.value;
      }
    }
  }
  ...
  public orgChartAligmentChange(args: ToolbarClickEventArgs): void {
    let diagram: Diagram = this.selectedItem.selectedDiagram as Diagram;
    let commandType: string = (args as any).item.tooltipText
      .replace(/[" "]/g, "")
      .toLowerCase();
    switch (commandType) {
      case "alignleft":
